...
const Checkout = () => {
  ...
  const price = searchParams.get("price") || "1,599";
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-orange-500/30">
      ...
      >
        ...
        {/* LEFT SIDE - Summary */}
        <div className="flex-1 p-6 sm:p-8 md:p-16 border-b md:border-b-0 md:border-r border-white/10 bg-[#0a0a0a]">
          <button 
            onClick={() => navigate(-1)}
            className="flex items-center text-white/40 hover:text-orange-500 transition-all mb-8 sm:mb-12 text-sm font-medium group"
          >
          ...
          <header className="mb-8 sm:mb-12">
            <div className="flex items-center gap-3 mb-6 sm:mb-8">
              ...
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight font-[Montserrat]">Graphobot</h1>
            </div>
            
            <p className="text-white/50 font-medium mb-2 uppercase tracking-wider text-xs">Subscribe to {planName}</p>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl sm:text-5xl font-bold font-[Montserrat] text-white">₹{price}</span>
              <span className="text-white/40 font-medium italic text-sm sm:text-base">/ {interval}</span>
            </div>
          </header>
          ...
          <div className="space-y-5 sm:space-y-6">
            ...
            <div className="pt-5 sm:pt-6 border-t border-white/10 space-y-3 sm:space-y-4">
              ...
              <div className="flex justify-between text-sm">
                ...
                <p className="text-white/30 italic text-xs sm:text-sm">Calculated at next step</p>
              </div>
            </div>

            <div className="pt-5 sm:pt-6 border-t border-white/10 flex justify-between items-baseline">
              <p className="text-white font-semibold text-base sm:text-lg">Total due today</p>
              <p className="text-2xl sm:text-3xl font-bold text-white font-[Montserrat]">₹{price}</p>
            </div>
          </div>
        </div>

        {/* RIGHT SIDE - Payment Form */}
        <div className="flex-1 p-6 sm:p-8 md:p-16 bg-black">
          <div className="max-w-[400px] mx-auto">
            ...
            <section className="mb-8 sm:mb-10">
              ...
              <div className="border border-white/10 rounded-2xl overflow-hidden bg-[#0a0a0a]">
                ...
                <div 
                  ...
                  className={`p-4 sm:p-5 flex items-center justify-between cursor-pointer transition-all ${paymentMethod === 'card' ? 'bg-orange-500/10 border-orange-500/20' : 'hover:bg-white/5'}`}
                >
                ...
                <div 
                  ...
                  className={`p-4 sm:p-5 border-t border-white/5 flex items-center gap-3 sm:gap-4 cursor-pointer transition-all ${paymentMethod === 'upi' ? 'bg-orange-500/10' : 'hover:bg-white/5'}`}
                >
                  ...
                  <span className="text-sm font-semibold tracking-wider italic">UPI / GPay / PhonePe</span>
                </div>
              </div>
            </section>
            ...
            <motion.button 
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full bg-orange-500 text-white font-bold py-3.5 sm:py-4 rounded-xl shadow-lg shadow-orange-900/20 transition-all text-sm sm:text-base mb-6 font-[Montserrat] uppercase tracking-wider"
            >
              Complete Purchase
            </motion.button>
